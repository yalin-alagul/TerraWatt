...
export default async function CountryPage({ params }: { params: Promise<{ code: string }> }) {
  ...
  const predictions = await fetchPredictions(code);
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        {hasData && (
          <div className="flex gap-4">
            <a 
              href={`https://robertkolej.com/api/energy/mix?country_code=${code}`}
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-slate-900 border border-slate-800 p-6 rounded-xl">
              ...
              <p className="text-3xl font-mono text-green-400">{latestYear?.battery_storage_mwh?.toLocaleString() || 0} <span className="text-sm text-slate-600">MWh</span></p>
            </div>
            <div className="bg-slate-900 border border-slate-800 p-6 rounded-xl">
              <h4 className="text-slate-500 text-xs uppercase tracking-widest font-bold mb-1">Pumped Hydro</h4>
              <p className="text-3xl font-mono text-blue-400">{latestYear?.pumped_hydro_mwh?.toLocaleString() || 0} <span className="text-sm text-slate-600">MWh</span></p>
            </div>
            <div className="bg-slate-900 border border-slate-800 p-6 rounded-xl">
              <h4 className="text-slate-500 text-xs uppercase tracking-widest font-bold mb-1">Region</h4>
              <p className="text-3xl font-bold text-slate-200">{(latestYear as any)?.region || 'Other'}</p>
            </div>
          </div>
